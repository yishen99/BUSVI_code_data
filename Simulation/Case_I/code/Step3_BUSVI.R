...
## set seed for BUSVI in simulation case I
seed = 5
set.seed(seed)
...
while(TRUE){
  ...
  if(t %% 50 == 0) cat("######## THE CAVI IS RUNNING WHEN t = ", t, "########", "\n")
  t = t + 1
}
...
## BUSVI clustering labels
Z_BUSVI <- list()
for(b in 1:B){
  Z_BUSVI[[b]] = rep(NA, times = n_vec[b])
  for(i in 1:n_vec[b]){
    Z_BUSVI[[b]][i] = which.max(varphi_t[[b]][i, ])
  }
}
...
Y_BUSVI[[1]] = Y[[1]]
for(b in 2:B){
  Y_BUSVI[[b]] <- matrix(NA, nrow = n_vec[b], ncol = G)
  for(i in 1:n_vec[b]){
    Y_BUSVI[[b]][i, ] = alpha_hat + xi_hat[, Z_BUSVI[[b]][i]] + (Y[[b]][i, ]-alpha_hat-xi_hat[, Z_BUSVI[[b]][i]]-gamma_hat[b, ]) / (sigma_hat[b, ]/sigma_hat[1, ])
  }
}
...
cat(paste0("  The BUSVI procedure takes: ", Running_Time, " seconds", "\n"))


###################### ARI Computing #########################

ARI_BUSVI <- data.frame(Batch1 = round(ARI(Z_real[[1]], Z_BUSVI[[1]]), 3), 
                        Batch2 = round(ARI(Z_real[[2]], Z_BUSVI[[2]]), 3), 
                        Batch3 = round(ARI(Z_real[[3]], Z_BUSVI[[3]]), 3),
                        Overall = round(ARI(unlist(Z_real), unlist(Z_BUSVI)), 3))

## Save ARI
write.csv(ARI_BUSVI, row.names = "BUSVI ARI", file = "../result_data/ARI_BUSVI.csv")
...
save(Y_BUSVI, file = "../result_data/Y_BUSVI.RData")
save(Z_BUSVI, file = "../result_data/Z_BUSVI.RData")
